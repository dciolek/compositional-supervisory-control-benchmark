(define
  ...
  (:constants
    $HeightMonitor-1 $HeightMonitor-2 $HeightMonitor-0 $HeightMonitor-4 $RampMonitor $ResponseMonitor $HeightMonitor-3 $Airplane-0 - LTS
    $-1 $0 $1 $2 $3 $4 $5 $6 $7 $8 - State
    $descend-0-2 $control-all $approach-0 $descend-0-3 $descend-0-4 $land-0 $land-crash $requestLand-0 $descend-0-1 $extendFlight-0 $descend-0-0 - Label
  ...
  (:action do$descend-0-1
    ...
    :effect
      (and
        ...
        (when (at $8 $Airplane-0)
          (and (not (at $8 $Airplane-0)) (at $5 $Airplane-0))
        )
      )
  )
