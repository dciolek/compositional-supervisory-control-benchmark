(define
  ...
  (:constants
    $HeightMonitor-1 $Airplane-1 $HeightMonitor-0 $RampMonitor $ResponseMonitor $Airplane-0 - LTS
    $-1 $0 $1 $2 $3 $4 $5 - State
    $approach-1 $control-all $approach-0 $descend-1-1 $land-crash $extendFlight-0 $extendFlight-1 $air-crash-0 $air-crash-1 $land-1 $land-0 $requestLand-0 $descend-1-0 $requestLand-1 $descend-0-1 $descend-0-0 - Label
  ...
  (:action do$descend-1-1
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $0 $HeightMonitor-1)
          (and (not (at $0 $HeightMonitor-1)) (at $3 $HeightMonitor-1))
        )
        (when (at $1 $HeightMonitor-1)
  ...
  (:action do$descend-1-0
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $3 $HeightMonitor-1)
          (and (not (at $3 $HeightMonitor-1)) (at $0 $HeightMonitor-1))
        )
        (when (at $2 $Airplane-1)
  ...
  (:action do$descend-0-1
    ...
    :effect
      (and
        ...
        (when (at $3 $HeightMonitor-1)
          (and (not (at $3 $HeightMonitor-1)) (at $4 $HeightMonitor-1))
        )
        (when (at $2 $ResponseMonitor)
