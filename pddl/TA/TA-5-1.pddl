(define
  ...
  (:constants
    ...
    $agency-request $query-fail-1 $unavailable-1 $unavailable-0 $query-fail-0 $unavailable-3 $unavailable-2 $steps-4-0 $unavailable-4 $committed-0 $committed-1 $query-fail-3 $query-fail-2 $query-fail-4 $steps-0-0 $steps-1-0 $steps-3-0 $query-4 $query-3 $purchase-fail-4 $query-2 $purchase-fail-3 $query-1 $query-0 $steps-2-0 $agency-fail $cancel-4 $uncommitted-3 $uncommitted-4 $uncommitted-0 $uncommitted-1 $uncommitted-2 $purchase-fail-1 $purchase-fail-2 $purchase-fail-0 $purchase-2 $cancel-3 $cancel-2 $purchase-1 $cancel-1 $purchase-4 $purchase-3 $cancel-0 $purchase-0 $agency-succ $committed-4 $committed-3 $available-0 $committed-2 $available-1 $available-2 $available-3 $available-4 $query-succ-4 $purchase-succ-3 $query-succ-3 $query-succ-2 $purchase-succ-4 $purchase-succ-1 $query-succ-1 $query-succ-0 $purchase-succ-2 $purchase-succ-0 - Label
  ...
  (:action do$agency-fail
    ...
    :effect
      (and
        ...
        (when (at $6 $Service-4)
          (and (not (at $6 $Service-4)) (at $0 $Service-4))
        )
        (when (at $8 $Service-4)
  ...
  (:action do$committed-4
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $5 $Service-4)
          (and (not (at $5 $Service-4)) (at $8 $Service-4))
        )
      )
  )
