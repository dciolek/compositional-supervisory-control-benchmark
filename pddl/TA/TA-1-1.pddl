(define
  ...
  (:constants
    ...
    $agency-request $query-fail-0 $unavailable-0 $agency-succ $query-0 $committed-0 $cancel-0 $purchase-0 $agency-fail $available-0 $steps-0-0 $uncommitted-0 $query-succ-0 $purchase-fail-0 $purchase-succ-0 - Label
  ...
  (:action do$unavailable-0
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $1 $Service-0)
          (and (not (at $1 $Service-0)) (at $9 $Service-0))
        )
      )
  )
