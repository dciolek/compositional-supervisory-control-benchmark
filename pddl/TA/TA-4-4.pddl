(define
  ...
  (:constants
    ...
    $query-fail-1 $unavailable-1 $agency-request $unavailable-0 $query-fail-0 $steps-3-2 $unavailable-3 $steps-3-3 $unavailable-2 $committed-0 $committed-1 $query-fail-3 $query-fail-2 $steps-0-3 $steps-0-1 $steps-0-2 $steps-1-3 $steps-0-0 $steps-1-2 $steps-1-1 $steps-1-0 $steps-3-1 $steps-3-0 $query-3 $query-2 $purchase-fail-3 $query-1 $query-0 $steps-2-1 $steps-2-2 $steps-2-0 $select-3 $agency-fail $select-2 $uncommitted-3 $steps-2-3 $uncommitted-0 $uncommitted-1 $uncommitted-2 $purchase-fail-1 $purchase-fail-2 $purchase-fail-0 $purchase-2 $cancel-3 $cancel-2 $purchase-1 $cancel-1 $purchase-3 $cancel-0 $purchase-0 $select-0 $select-1 $agency-succ $committed-3 $available-0 $committed-2 $available-1 $available-2 $available-3 $purchase-succ-3 $query-succ-3 $query-succ-2 $purchase-succ-1 $query-succ-1 $purchase-succ-2 $query-succ-0 $purchase-succ-0 - Label
  ...
  (:action do$steps-3-2
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $2 $Service-3)
          (and (not (at $2 $Service-3)) (at $14 $Service-3))
        )
      )
  )
  ...
  (:action do$select-3
    ...
    :effect
      (and
        ...
        (when (at $13 $Service-3)
          (and (not (at $13 $Service-3)) (at $10 $Service-3))
        )
      )
  )
  ...
  (:action do$query-succ-3
    ...
    :effect
      (and
        ...
        (when (at $14 $Service-3)
          (and (not (at $14 $Service-3)) (at $13 $Service-3))
        )
        (when (at $1 $ServiceMonitor-3)
