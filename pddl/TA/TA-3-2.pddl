(define
  ...
  (:constants
    ...
    $unavailable-1 $query-fail-1 $agency-request $query-fail-0 $unavailable-0 $unavailable-2 $order-2 $order-1 $order-0 $committed-0 $committed-1 $query-fail-2 $steps-0-1 $steps-0-0 $steps-1-1 $steps-1-0 $query-2 $query-1 $query-0 $steps-2-1 $steps-2-0 $reserve-1 $reserve-0 $agency-fail $select-2 $uncommitted-0 $uncommitted-1 $uncommitted-2 $purchase-fail-1 $purchase-fail-2 $purchase-fail-0 $purchase-2 $cancel-2 $purchase-1 $cancel-1 $cancel-0 $purchase-0 $select-0 $select-1 $reserve-2 $end $agency-succ $available-0 $committed-2 $available-1 $available-2 $query-succ-2 $query-succ-1 $purchase-succ-1 $purchase-succ-2 $query-succ-0 $purchase-succ-0 - Label
  ...
  (:action do$unavailable-2
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $1 $Service-2)
          (and (not (at $1 $Service-2)) (at $13 $Service-2))
        )
      )
  )
  ...
  (:action do$select-2
    ...
    :effect
      (and
        ...
        (not (status idle))
        (when (at $12 $Service-2)
          (and (not (at $12 $Service-2)) (at $4 $Service-2))
        )
      )
  )
