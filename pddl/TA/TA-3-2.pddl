(define
  ...
  (:constants
    ...
    $agency-request $query-fail-1 $unavailable-1 $unavailable-0 $query-fail-0 $unavailable-2 $purchase-2 $purchase-1 $cancel-2 $committed-0 $cancel-1 $cancel-0 $committed-1 $query-fail-2 $purchase-0 $steps-0-1 $steps-0-0 $select-0 $steps-1-1 $steps-1-0 $select-1 $query-2 $query-1 $agency-succ $query-0 $steps-2-1 $steps-2-0 $agency-fail $select-2 $available-0 $committed-2 $available-1 $available-2 $uncommitted-0 $uncommitted-1 $uncommitted-2 $query-succ-2 $purchase-fail-1 $purchase-succ-1 $query-succ-1 $query-succ-0 $purchase-succ-2 $purchase-fail-2 $purchase-succ-0 $purchase-fail-0 - Label
  ...
  (:action do$select-2
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $10 $Service-2)
          (and (not (at $10 $Service-2)) (at $4 $Service-2))
        )
      )
  )
