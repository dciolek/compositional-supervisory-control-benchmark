(define
  ...
  (:constants
    ...
    $agency-request $unavailable-1 $query-fail-1 $unavailable-0 $query-fail-0 $agency-succ $query-1 $query-0 $purchase-1 $committed-0 $cancel-1 $cancel-0 $committed-1 $purchase-0 $agency-fail $available-0 $available-1 $steps-0-0 $steps-1-0 $uncommitted-0 $uncommitted-1 $query-succ-1 $purchase-succ-1 $purchase-fail-1 $query-succ-0 $purchase-succ-0 $purchase-fail-0 - Label
  ...
  (:action do$unavailable-1
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $1 $Service-1)
          (and (not (at $1 $Service-1)) (at $9 $Service-1))
        )
      )
  )
