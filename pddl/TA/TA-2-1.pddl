(define
  ...
  (:constants
    ...
    $agency-request $query-fail-1 $unavailable-1 $unavailable-0 $query-fail-0 $order-1 $order-0 $purchase-1 $committed-0 $cancel-1 $committed-1 $cancel-0 $purchase-0 $steps-0-0 $steps-1-0 $end $query-1 $agency-succ $query-0 $reserve-1 $reserve-0 $agency-fail $available-0 $available-1 $uncommitted-0 $uncommitted-1 $purchase-fail-1 $purchase-succ-1 $query-succ-1 $query-succ-0 $purchase-fail-0 $purchase-succ-0 - Label
