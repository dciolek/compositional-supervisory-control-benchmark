(define
  ...
  (:constants
    ...
    $agency-request $query-fail-1 $unavailable-1 $unavailable-0 $query-fail-0 $order-1 $order-0 $purchase-1 $committed-0 $cancel-1 $committed-1 $cancel-0 $purchase-0 $steps-0-3 $steps-0-1 $steps-0-2 $steps-1-3 $steps-0-0 $steps-1-2 $select-0 $steps-1-1 $steps-1-0 $select-1 $end $query-1 $agency-succ $query-0 $reserve-1 $reserve-0 $agency-fail $available-0 $available-1 $uncommitted-0 $uncommitted-1 $purchase-fail-1 $purchase-succ-1 $query-succ-1 $query-succ-0 $purchase-succ-0 $purchase-fail-0 - Label
  ...
  (:action do$query-fail-1
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $17 $Service-1)
          (and (not (at $17 $Service-1)) (at $0 $Service-1))
        )
        (when (at $0 $AgencyMonitor)
  ...
  (:action do$unavailable-1
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $1 $Service-1)
          (and (not (at $1 $Service-1)) (at $17 $Service-1))
        )
      )
  )
